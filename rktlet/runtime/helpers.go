...
func generateAppAddCommand(req *runtimeApi.CreateContainerRequest, imageID string) ([]string, error) {
	...
	for _, mnt := range config.GetMounts() {
		...
		if err := createHostPathVolume(mnt); err != nil {
...
// createHostPathVolume creates the source dir for Mount since rkt doesn't create it if it doesn't exist
func createHostPathVolume(mount *runtimeApi.Mount) (err error) {
